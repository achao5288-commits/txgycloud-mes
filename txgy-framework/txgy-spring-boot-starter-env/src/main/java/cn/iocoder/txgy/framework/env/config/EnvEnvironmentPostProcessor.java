...
import cn.iocoder.txgy.framework.env.core.util.EnvUtils;
...
import static cn.iocoder.txgy.framework.env.core.util.EnvUtils.HOST_NAME_VALUE;

/**
 * 多环境的 {@link EnvEnvironmentPostProcessor} 实现类
 * 将 txgy.env.tag 设置到 nacos 等组件对应的 tag 配置项，当且仅当它们不存在时
 *
 * @author OPENLAB BS
 */
public class EnvEnvironmentPostProcessor implements EnvironmentPostProcessor {
    ...
    public void postProcessEnvironment(ConfigurableEnvironment environment, SpringApplication application) {
        ...
        // 1.1 如果没有 txgy.env.tag 配置项，则不进行配置项的修改
