...
import {
  createOccupationalHazard,
  getOccupationalHazard,
  updateOccupationalHazard,
...
const getTitle = computed(() => {
  ...
  return formType.value === 'update'
    ? $t('ui.actionTitle.edit', ['职业病危害检测'])
    : $t('ui.actionTitle.create', ['职业病危害检测']);
});
...
const [Modal, modalApi] = useVbenModal({
  async onConfirm() {
    ...
    modalApi.lock();
    // 提交表单
    const data = (await formApi.getValues()) as MesSetOccupationalHazardApi.OccupationalHazard;
    try {
      await (formType.value === 'update'
        ? updateOccupationalHazard(data)
        : createOccupationalHazard(data));
      // 关闭并提示
      await modalApi.close();
      emit('success');
  ...
  async onOpenChange(isOpen: boolean) {
    ...
    modalApi.lock();
    try {
      const row = await getOccupationalHazard(data.id);
      // 设置到 values
