...
const getTitle = computed(() => {
  ...
  return formType.value === 'update'
    ? $t('ui.actionTitle.edit', ['检测标准管理'])
    : $t('ui.actionTitle.create', ['检测标准管理']);
});
...
const [Modal, modalApi] = useVbenModal({
  async onConfirm() {
    ...
    modalApi.lock();
    // 提交表单
    const data = (await formApi.getValues()) as MesSetStandardApi.Standard;
    try {
  ...
  async onOpenChange(isOpen: boolean) {
    ...
    modalApi.lock();
    try {
      const row = await getStandard(data.id);
      // 设置到 values
