...
const getTitle = computed(() =>
  formType.value === 'update'
    ? $t('ui.actionTitle.edit', ['应急物资'])
    : $t('ui.actionTitle.create', ['应急物资']),
);
...
const [Modal, modalApi] = useVbenModal({
  async onConfirm() {
    ...
    try {
      await (formType.value === 'update'
        ? updateEmergencyMaterial(data)
        : createEmergencyMaterial(data));
  ...
  async onOpenChange(isOpen: boolean) {
    ...
    modalApi.lock();
    try {
      const row = await getEmergencyMaterial(data.id);
