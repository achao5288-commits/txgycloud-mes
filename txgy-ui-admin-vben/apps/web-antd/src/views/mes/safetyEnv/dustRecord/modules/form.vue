...
const getTitle = computed(() => {
  ...
  return formType.value === 'update'
    ? $t('ui.actionTitle.edit', ['粉尘浓度检测'])
    : $t('ui.actionTitle.create', ['粉尘浓度检测']);
});
...
const [Modal, modalApi] = useVbenModal({
  async onConfirm() {
    ...
    modalApi.lock();
    // 提交表单
    const data = (await formApi.getValues()) as MesSetDustRecordApi.DustRecord;
    try {
      await (formType.value === 'update'
        ? updateDustRecord(data)
        : createDustRecord(data));
      // 关闭并提示
      await modalApi.close();
      emit('success');
  ...
  async onOpenChange(isOpen: boolean) {
    ...
    modalApi.lock();
    try {
      const row = await getDustRecord(data.id);
      // 设置到 values
