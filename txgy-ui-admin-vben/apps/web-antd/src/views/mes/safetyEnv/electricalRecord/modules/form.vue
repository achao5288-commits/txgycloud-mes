...
import {
  createElectricalRecord,
  getElectricalRecord,
  updateElectricalRecord,
...
const getTitle = computed(() => {
  ...
  return formType.value === 'update'
    ? $t('ui.actionTitle.edit', ['电气安全检测'])
    : $t('ui.actionTitle.create', ['电气安全检测']);
});
...
const [Modal, modalApi] = useVbenModal({
  async onConfirm() {
    ...
    modalApi.lock();
    // 提交表单
    const data = (await formApi.getValues()) as MesSetElectricalRecordApi.ElectricalRecord;
    try {
      await (formType.value === 'update'
        ? updateElectricalRecord(data)
        : createElectricalRecord(data));
      // 关闭并提示
      await modalApi.close();
      emit('success');
  ...
  async onOpenChange(isOpen: boolean) {
    ...
    modalApi.lock();
    try {
      const row = await getElectricalRecord(data.id);
      // 设置到 values
