...
export namespace MesSetPressureVesselApi {
  ...
  export interface PressureVessel {
    id?: number; // 编号
    recordNo?: string; // 检验记录/报告编号 PV-YYYY-NNN
    planId?: number; // 关联检测计划编号
    deviceId?: number; // 关联设备编号(特种设备台账)
    vesselRegNo?: string; // 压力容器使用登记证号(特种设备注册代码)
    wallThickness?: number; // 壁厚测定最小壁厚 mm
    ndtMethods?: string; // 无损检测方法：UT/RT/MT/PT(多选逗号分隔)
    ndtResults?: string; // 无损检测结果JSON文本如{UT:PASS,MT:PASS}
    safetyValveOk?: boolean; // 安全阀校验合格：1是/0否
    ...
    inspectOrg?: string; // 检验机构(需资质)
    nextInspectDate?: string; // 下次检验日期
    reportFileUrl?: string; // 检验报告文件URL
