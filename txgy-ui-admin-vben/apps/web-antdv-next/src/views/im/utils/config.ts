 ...
 *
 * 标注「后端镜像」的常量与 txgy.im.* 配置默认值一致；调整后端配置时需同步修改这里
 */

// ==================== 后端镜像（与 ImProperties 默认值对齐） ====================

/** 群最大成员人数（对齐 txgy.im.group.max-member） */
export const GROUP_MAX_MEMBER = 500

/** 单群管理员人数上限（对齐 txgy.im.group.admin-max-count） */
export const GROUP_ADMIN_MAX_COUNT = 3

/**
 * 是否启用私聊已读功能（对齐 txgy.im.message.private-read-enabled）
 *
...
/**
 * 是否启用群聊已读功能（含群消息回执，对齐 txgy.im.message.group-read-enabled）
 *
 ...
 * 实际超时阈值由后端 txgy.im.rtc.invite-timeout-minutes 决定，前端仅决定触发频率
 */
