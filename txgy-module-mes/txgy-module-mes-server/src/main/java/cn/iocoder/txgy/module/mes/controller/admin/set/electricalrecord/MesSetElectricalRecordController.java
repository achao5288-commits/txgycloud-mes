...
@Tag(name = "管理后台 - MES 安全环保检测-电气安全检查")
@RestController
...
public class MesSetElectricalRecordController {
    ...
    public CommonResult<MesSetElectricalRecordRespVO> getElectricalRecord(@RequestParam("id") Long id) {
        MesSetElectricalRecordDO obj = electricalrecordService.getElectricalRecord(id);
        return success(BeanUtils.toBean(obj, MesSetElectricalRecordRespVO.class));
    }

    @GetMapping("/page")
    @Operation(summary = "获得电气安全检查分页")
    @PreAuthorize("@ss.hasPermission('mes:set-electrical-record:query')")
    public CommonResult<PageResult<MesSetElectricalRecordRespVO>> getElectricalRecordPage(
            @Valid MesSetElectricalRecordPageReqVO pageReqVO) {
