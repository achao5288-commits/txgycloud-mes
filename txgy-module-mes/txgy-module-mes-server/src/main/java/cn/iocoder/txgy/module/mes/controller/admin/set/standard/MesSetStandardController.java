...
public class MesSetStandardController {
    ...
    public CommonResult<MesSetStandardRespVO> getStandard(@RequestParam("id") Long id) {
        MesSetStandardDO obj = standardService.getStandard(id);
        return success(BeanUtils.toBean(obj, MesSetStandardRespVO.class));
    }

    @GetMapping("/page")
    @Operation(summary = "获得检测标准分页")
    @PreAuthorize("@ss.hasPermission('mes:set-standard:query')")
    public CommonResult<PageResult<MesSetStandardRespVO>> getStandardPage(
            @Valid MesSetStandardPageReqVO pageReqVO) {
