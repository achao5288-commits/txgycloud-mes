...
import cn.iocoder.txgy.framework.web.core.util.WebFrameworkUtils;
import cn.iocoder.txgy.module.bpm.controller.admin.definition.vo.model.BpmModelMetaInfoVO;
import cn.iocoder.txgy.module.bpm.controller.admin.task.vo.task.*;
import cn.iocoder.txgy.module.bpm.convert.task.BpmTaskConvert;
...
import cn.iocoder.txgy.module.bpm.enums.definition.*;
import cn.iocoder.txgy.module.bpm.enums.task.*;
...
import cn.iocoder.txgy.module.bpm.service.comment.BpmCommentService;
import cn.iocoder.txgy.module.bpm.service.definition.BpmFormService;
import cn.iocoder.txgy.module.bpm.service.definition.BpmModelService;
...
import cn.iocoder.txgy.module.bpm.service.message.dto.BpmMessageSendWhenTaskTimeoutReqDTO;
...
import static cn.iocoder.txgy.module.bpm.framework.flowable.core.util.BpmnModelUtils.*;
...
//        }
    ...
    public void rejectTask(Long userId, @Valid BpmTaskRejectReqVO reqVO) {
        ...
        // 疑问：为什么要标记未通过呢？
        // 回答：例如说 A 任务被向前加签除 B 任务时，B 任务被审批不通过，此时 A 会被取消。而 txgy-ui-admin-vue3 不展示“已取消”的任务，导致展示不出审批不通过的细节。
    ...
    public void returnTask(Long userId, BpmnModel bpmnModel, Task currentTask, FlowElement targetElement, BpmTaskReturnReqVO reqVO) {
        ...
        // ③ moveActivityIdsToSingleActivityId 使用遇到问题， 相关 issue https://gitee.com/zhijiantianya/txgy-cloud/issues/IJM8MS
        ...
        // ⑤ 使用 moveExecutionsToSingleActivityId 方法进行回退操作时，如果是多实例的用户任务【OPENLAB BS用户任务默认为多实例】，不会删除多实例任务的根数据 ACT_RU_EXECUTION
    ...
    public void delegateTask(Long userId, BpmTaskDelegateReqVO reqVO) {
        ...
        // 特殊：如果已经被委派（owner 非空），则不需要更新 owner：https://gitee.com/zhijiantianya/txgy-cloud/issues/ICJ153
    ...
    public void transferTask(Long userId, BpmTaskTransferReqVO reqVO) {
        ...
        // 3.1 设置任务所有人 (owner) 为原任务的处理人 (assignee)
        // 特殊：如果已经被转派（owner 非空），则不需要更新 owner：https://gitee.com/zhijiantianya/txgy-cloud/issues/ICJ153
        if (StrUtil.isEmpty(task.getOwner())) {
            taskService.setOwner(taskId, task.getAssignee());
        }
    ...
    public void processTaskCreated(Task task) {
        ...
        TransactionSynchronizationManager.registerSynchronization(new TransactionSynchronization() {
             ...
             * 参见 <a href="https://gitee.com/zhijiantianya/txgy-cloud/issues/IB7V7Q">issue</a> 反馈
             */
            @Override
