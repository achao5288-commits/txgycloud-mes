...
import cn.iocoder.txgy.module.report.controller.admin.goview.vo.data.GoViewDataRespVO;
...
public class GoViewDataServiceImplTest extends BaseDbUnitTest {
    ...
    public void testGetDataBySQL() {
        ...
        when(sqlRowSet.getObject("name")).thenReturn("OPENLAB BS").thenReturn("OPENLAB BS");
        ...
        assertEquals("OPENLAB BS", dataBySQL.getSource().get(0).get("name"));
        assertEquals(2, dataBySQL.getSource().get(1).size());
        assertEquals(2L, dataBySQL.getSource().get(1).get("id"));
        assertEquals("OPENLAB BS", dataBySQL.getSource().get(1).get("name"));
    }
